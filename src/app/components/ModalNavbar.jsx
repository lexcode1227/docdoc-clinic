...
const ModalNavbar = ({handleMenu, closeMenuMobile}) => {
  const navList = [
    {
        name: "HOME",
        to: "/",
    ...
    <section className='h-[93vh] z-10 flex flex-col gap-40 justify-evenly mx-5 py-12 md:w-3/4'>
      ...
      <div className="h-1/3 flex flex-col items-center gap-8 ">
        <nav className="flex flex-col items-center gap-3">
          {navList.map((item)=> {
            return (
                <Link key={item.name} href={item.to} onClick={closeMenuMobile} className="text-[#5b5b5b] text-center font-medium hover:text-[#2b2e35] hover:font-bold">{item.name}</Link>
            )
          })}
        </nav>
        <Link href="/appointment" className="transition-colors w-[250px] text-center bg-[#EEEFFF] text-[#032FF2] p-3 border border-transparent rounded-[10px] hover:border-[#032FF2] hover:border ">Get Appointment</Link> 
      </div>
